...
function NavIcon({ name }: { name: string }) {
  const icons: Record<string, string> = {
    ...
    offers: "M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z",
    ...
    questionnaires: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01",
...
export function AdminSidebar({ onNavigate }: AdminSidebarProps) {
  ...
  return (
    <aside className="flex h-full w-64 flex-col border-r border-[var(--admin-border)] bg-[var(--admin-sidebar)]">
      <div className="border-b border-[var(--admin-border)] px-5 py-5">
        <Link href="/admin" className="block" onClick={onNavigate}>
          <span className="text-lg font-bold tracking-tight text-[var(--admin-accent)]">Sodapops</span>
          <span className="mt-0.5 block text-xs text-[var(--admin-muted)]">Admin Portal</span>
        </Link>
      </div>
                ...
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors ${
                    active
                      ...
                      : "text-[var(--admin-muted)] hover:bg-[var(--admin-surface-hover)] hover:text-[var(--admin-text)]"
                  }`}
                >
