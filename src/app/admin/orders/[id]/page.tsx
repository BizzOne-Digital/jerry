...
export default function AdminOrderDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const qRes = await fetch(`/api/admin/questionnaires?orderId=${id}`);
      const qData = await qRes.json();
      if (qRes.ok && qData.items?.length) {
        setQuestionnaire(qData.items[0]);
      }
      ...
      {questionnaire && (
        <div className="admin-card mb-6 p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h2 className="font-semibold">Collector Profile</h2>
            <Link
              href={`/admin/questionnaires/${questionnaire._id}`}
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {questionnaire.favoriteSports?.length && (
              <div>
                <p className="text-sm text-[var(--admin-muted)]">Sports</p>
                <p>{questionnaire.favoriteSports.join(", ")}</p>
              </div>
            )}
            {questionnaire.itemsToCollect?.length && (
              <div>
                <p className="text-sm text-[var(--admin-muted)]">Collects</p>
                <p>{questionnaire.itemsToCollect.join(", ")}</p>
              </div>
            )}
            {questionnaire.favoriteTeams?.length && (
              <div>
                <p className="text-sm text-[var(--admin-muted)]">Teams</p>
                <p>{questionnaire.favoriteTeams.join(", ")}</p>
              </div>
            )}
            {questionnaire.favoritePlayers?.length && (
              <div>
                <p className="text-sm text-[var(--admin-muted)]">Players</p>
                <p>{questionnaire.favoritePlayers.join(", ")}</p>
              </div>
            )}
          </div>
          {questionnaire.collectingGoals && (
            <p className="mt-4 text-sm text-[var(--admin-muted)]">
              <span className="text-[var(--admin-text)]">Goals:</span> {questionnaire.collectingGoals}
            </p>
          )}
        </div>
      )}
